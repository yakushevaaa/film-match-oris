...
export const AuthForm: FC<FormProps> = ({ type }) => {
  ...
  const mainInputs = inputs.filter((input) => !passwordInputs.includes(input));
      ...
      {isRegister ? (
        <div className={styles.password_container}>
          {passwordInputs.map((input) => (
            <Input key={input.id} {...input} className={styles.passwordInput} />
          ))}
        </div>
      ) : (
        passwordInputs.map((input) => <Input key={input.id} {...input} />)
      )}
