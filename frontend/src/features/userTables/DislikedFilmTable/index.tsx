...
export const DislikedFilmTable = () => {
  ...
  const fetchLikedFilms = async () => {
    ...
    setError("");
    try {
      const res = await axiosSettings.get("/Film/AllDislikedFilms");
  ...
  const handleRemoveLike = async (row: MRT_Row<Film>) => {
    try {
      await axiosSettings.post(`/Film/Dislike/${row.original.id}`);
      setFilms((prev) => prev.filter((f) => f.id !== row.original.id));
    } catch {
      setError("Не удалось убрать дизлайк");
    }
  };
  ...
  const columns: MRT_ColumnDef<Film>[] = [
    ...
    { accessorKey: "shortDescription", header: "Описание", size: 300 },
    {
      ...
      Cell: ({ row }) => (
        <button
          className={styles.removeDislikeButton}
